...
it('links ProductScopedOverrides as an extender of Product after EntityMetadataFactory::linkExtendersFrom runs against the discovered entity list', function (): void {
...
it('links CategoryScopedOverrides as an extender of Category after EntityMetadataFactory::linkExtendersFrom runs against the discovered entity list', function (): void {
    $factory = new EntityMetadataFactory();
    $factory->linkExtendersFrom([
        Category::class,
...
it('merges the scopes column into the catalog_products parent table when SchemaRegistry::registerEntities() runs with both Product and ProductScopedOverrides', function (): void {
    $factory = new EntityMetadataFactory();
...
it('attaches a ProductScopedOverrides companion to a hydrated Product when the SELECT row includes the scopes column', function (): void {
    $factory = new EntityMetadataFactory();
...
it('silently omits the companion when a partial SELECT does not include the scopes column (documented gotcha; ScopeResolver falls back to raw value)', function (): void {
    $factory = new EntityMetadataFactory();
    $factory->linkExtenders(Product::class, [ProductScopedOverrides::class]);
    $hydrator = new EntityHydrator($factory);
...
it('attaches a ProductScopedOverrides companion with scopes=null when the row\'s scopes column is NULL (no overrides set)', function (): void {
    $factory = new EntityMetadataFactory();
    $factory->linkExtenders(Product::class, [ProductScopedOverrides::class]);
    $hydrator = new EntityHydrator($factory);
...
it('round-trips a scoped override on Product through save + re-fetch via the catalog ProductRepository (which selects all columns)', function (): void {
    $insertedScopes = null;
    $lastId = 0;
