...
it(
    'uses the Markommerce\\Locale\\ namespace for any future autoload (even though P2 ships no classes)',
    function (): void {
        $composerPath = dirname(__DIR__) . '/composer.json';
        $composer = json_decode(file_get_contents($composerPath), true);
...
it(
    'merges its locale axis into scope.axes when discovered alongside scope\'s own config/scope.php',
    function (): void {
