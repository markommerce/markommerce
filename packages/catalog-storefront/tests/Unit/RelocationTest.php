...
it('relocates Controller, Component, Context, Data, and Iteration source files from packages/catalog/src to packages/catalog-storefront/src with namespace Markommerce\\CatalogStorefront\\…', function (): void {
...
it('relocates the layout file from packages/catalog/layout/category_show.php to packages/catalog-storefront/layout/category_show.php', function (): void {
...
it('relocates Latte templates, CSS, JS, and package.json from packages/catalog/resources and packages/catalog to packages/catalog-storefront with the npm package renamed to @markommerce/catalog-storefront', function (): void {
    $storefrontRoot = dirname(__DIR__, 2);
...
it('rewrites every catalog::components/… template namespace reference to catalog-storefront::components/… in the moved layout file and moved tests', function (): void {
    $storefrontRoot = dirname(__DIR__, 2);
...
it('relocates CategoryControllerTest, CategoryLayoutTest, and ProductGridComponentTest into packages/catalog-storefront/tests with corrected dirname depths and ModuleManifest names', function (): void {
    $storefrontRoot = dirname(__DIR__, 2);
...
it('leaves packages/catalog/src with only Contracts, Entity, Enum, Exceptions, Repositories, and Services directories', function (): void {
...
it('asserts no Markommerce\\Catalog\\Controller, Markommerce\\Catalog\\Component, Markommerce\\Catalog\\Context, Markommerce\\Catalog\\Data, or Markommerce\\Catalog\\Iteration class exists after the move (grep the entire packages/ tree)', function (): void {
...
it('updates packages/catalog/tests/Unit/ScopeDecouplingTest.php to remove the two preservation blocks that target the now-moved CategoryControllerTest.php and CategoryLayoutTest.php (lines 96–116 of the existing file) so the catalog test suite stays green after the move', function (): void {
    ...
    expect($contents)->toContain("'preserves all non-scope test cases in CatalogSeederTreeTest and CatalogSeederTest");
});

it('updates packages/catalog/resources/js/package.test.ts (after relocation to packages/catalog-storefront/resources/js/) so the describe label and any embedded literals reference the new @markommerce/catalog-storefront npm package name', function (): void {
    $storefrontRoot = dirname(__DIR__, 2);
...
it('moves the catalog-scope ScopedProductGridComponent.php to catalog-storefront-scope with namespace Markommerce\\CatalogStorefrontScope\\Component (task 005)', function (): void {
    $catalogStorefrontScopeSrc = dirname(__DIR__, 3) . '/catalog-storefront-scope/src/Component/ScopedProductGridComponent.php';
...
it('moves the catalog-scope ScopedProductGridComponentTest.php to catalog-storefront-scope with updated namespace (task 005)', function (): void {
    $testFile = dirname(__DIR__, 3) . '/catalog-storefront-scope/tests/Unit/Component/ScopedProductGridComponentTest.php';
...
it('removes markommerce/catalog-storefront from packages/catalog-scope/composer.json require after task 005 moved ScopedProductGridComponent out', function (): void {
