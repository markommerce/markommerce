...
it('creates packages/catalog-storefront/composer.json declaring markommerce/catalog-storefront as a marko-module with the correct require list', function (): void {
...
it('includes markommerce/catalog, markommerce/layout, markommerce/frontend, markommerce/theme-blank, marko/routing, marko/view, marko/view-latte in the require block', function (): void {
    $composerPath = dirname(__DIR__, 2) . '/composer.json';
    $composer = json_decode(file_get_contents($composerPath), true);
...
it('creates packages/catalog-storefront/LICENSE, .gitattributes, src/.gitkeep, and tests/Pest.php matching the project scaffolding conventions', function (): void {
...
it('adds markommerce/catalog-storefront to the root composer.json require block alphabetically within the markommerce/* group', function (): void {
    ...
    $markommerceKeys = array_values(array_filter($keys, fn (string $k): bool => str_starts_with($k, 'markommerce/')));
...
it('adds Markommerce\\CatalogStorefront\\Tests\\ to the root composer.json autoload-dev.psr-4 block', function (): void {
    $rootComposerPath = dirname(__DIR__, 4) . '/composer.json';
    $composer = json_decode(file_get_contents($rootComposerPath), true);
...
it('passes composer validate on packages/catalog-storefront/composer.json', function (): void {
    $packageRoot = dirname(__DIR__, 2);
    $output = shell_exec('composer validate --no-check-publish ' . escapeshellarg($packageRoot . '/composer.json') . ' 2>&1');
