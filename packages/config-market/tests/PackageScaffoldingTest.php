...
it('declares config-market\'s name as markommerce/config-market with type marko-module in composer.json', function (): void {
...
it('requires markommerce/config-scope and markommerce/market as self.version dependencies in config-market\'s composer.json', function (): void {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode((string) file_get_contents($composerPath), true);
...
it('declares the Markommerce\\\\ConfigMarket\\\\ namespace mapped to src/ in config-market\'s autoload psr-4', function (): void {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode((string) file_get_contents($composerPath), true);
...
it('exposes a module.php that returns an array with require + a callable boot closure in config-market', function (): void {
