...
it('module.php registers ScopeResolutionPipeline via a closure binding that handles a missing logger gracefully', function (): void {
    $module = require dirname(__DIR__, 2) . '/module.php';
...
it('boot closure surfaces InvalidResolverConfigException from a misconfigured axis during Application initialize', function (): void {
...
it('boot closure pre-builds every axis resolver chain so misconfig throws at boot not on first request', function (): void {
    DefaultScopeGuard::reset();
...
it('the pipeline closure returns a working ScopeResolutionPipeline when marko log is not installed', function (): void {
    $module = require dirname(__DIR__, 2) . '/module.php';
