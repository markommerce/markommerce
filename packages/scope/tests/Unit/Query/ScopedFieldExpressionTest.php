...
it('no PHP file under packages/scope or packages/scope-pgsql imports ScopeSortRendererInterface or ScopeSortExpression', function (): void {
    ...
    foreach ($dirs as $dir) {
        ...
        foreach ($files as $file) {
            ...
            if (str_contains($contents, 'ScopeSortRendererInterface') || str_contains($contents, 'ScopeSortExpression')) {
...
it('ScopedFieldExpression preserves the candidateSignatures order as given (the constructor does not sort or normalize them; the order is the renderer\'s contract)', function (): void {
