...
it('returns an empty array from InMemoryScopedConfigStorage loadOverrides for an unknown config key', function (): void {
    $storage = new InMemoryScopedConfigStorage();

    expect($storage->loadOverrides('some/unknown.key'))->toBe([]);
});

it('stores and retrieves an override via InMemoryScopedConfigStorage saveOverride + loadOverrides round-trip', function (): void {
    $storage = new InMemoryScopedConfigStorage();
...
it('replaces an existing override under the same (key, signature) pair when saveOverride is called twice', function (): void {
    $storage = new InMemoryScopedConfigStorage();
    $storage->saveOverride('app/design.theme', 'locale:en', 'modern');
...
it('removes a single override via InMemoryScopedConfigStorage deleteOverride leaving other overrides intact', function (): void {
    $storage = new InMemoryScopedConfigStorage();
    $storage->saveOverride('app/design.theme', 'locale:en', 'modern');
