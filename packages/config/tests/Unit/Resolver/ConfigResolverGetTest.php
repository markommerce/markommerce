...
it('returns an instance of the preferred proxy class when a Preference is registered via PreferenceRegistry', function (): void {
    $preferenceRegistry = new PreferenceRegistry();
...
it('throws ProxyNotGeneratedException with a config:generate suggestion when the proxy class is missing', function (): void {
...
it('throws InvalidConfigClassException when the preferred class is not a subclass of the requested config class', function (): void {
    $preferenceRegistry = new PreferenceRegistry();
    ...
    $resolver = makeGetTestResolver([SampleConfig::class, UnrelatedConfig::class], preferenceRegistry: $preferenceRegistry);

    expect(fn () => $resolver->get(SampleConfig::class))
        ->toThrow(InvalidConfigClassException::class);
});

it('returns instances whose property hooks delegate through to ConfigResolver::resolved() for each field', function (): void {
    $storage = new InMemoryConfigStorage();
