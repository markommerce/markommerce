...
it('replaces an existing global value via setGlobal preserving any per-scope overrides on the same key', function (): void {
    $storage = new InMemoryConfigStorage();
    $writer = buildWriter($storage);
...
it('accepts SecretCipherInterface as a constructor dependency but does NOT invoke it for non-secret writes in this task\'s tests', function (): void {
    $storage = new InMemoryConfigStorage();
...
it('produces an identical row mutation when setOverride is called with null as when unsetOverride is called', function (): void {
...
it('produces an identical row mutation when setGlobal is called with null as when unsetGlobal is called', function (): void {
    $storageA = new InMemoryConfigStorage();
    $writerA = buildWriter($storageA);
...
it('returns successfully when unsetOverride is called for a signature that was never set (idempotent no-op via storage contract)', function (): void {
    $storage = new InMemoryConfigStorage();
    $writer = buildWriter($storage);
...
it('retries up to 3 times on compareAndSave version conflict before throwing StaleConfigWriteException', function (): void {
...
it('throws AxisNotDeclaredException when setOverride\'s signature uses an axis not declared on the property', function (): void {
    $storage = new InMemoryConfigStorage();
    $writer = buildWriter($storage);
