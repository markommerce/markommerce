...
it(
    'wires PluginInterceptor into the test container BEFORE resolving CategoryTreeService (otherwise the delete-guard test is meaningless)',
    function (): void {
...
it(
    'constructs the bridge ModuleManifest with the absolute filesystem path so PluginDiscovery can scan src/Plugins/',
    function (): void {
        $manifests = tier3BuildManifests();
...
it(
    'discovers and registers CategoryTreeServiceDeletePlugin via PluginDiscovery::discoverInModule against the bridge manifest',
    function (): void {
        $manifests = tier3BuildManifests();
        $manifest = tier3FindBridgeManifest($manifests);
