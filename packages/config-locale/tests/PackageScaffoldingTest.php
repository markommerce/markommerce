...
it('declares config-locale\'s name as markommerce/config-locale with type marko-module in composer.json', function (): void {
...
it('requires markommerce/config-scope and markommerce/locale as self.version dependencies in config-locale\'s composer.json', function (): void {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode((string) file_get_contents($composerPath), true);
...
it('declares the Markommerce\\\\ConfigLocale\\\\ namespace mapped to src/ in config-locale\'s autoload psr-4', function (): void {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode((string) file_get_contents($composerPath), true);
...
it('exposes a module.php that returns an array with require + a callable boot closure in config-locale', function (): void {
