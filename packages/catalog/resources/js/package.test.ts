...
describe('catalog package frontend wiring', () => {
  it('registers the catalog package with the frontend scanner via package.json markommerce.extension', () => {
    expect(fs.existsSync(packageJsonPath)).toBe(true);
    expect(markommerce).toBeDefined();
    expect(markommerce?.['extension']).toBe('./resources/js/index.ts');
  });

  it('provides an empty resources/js/index.ts in catalog ready for task 015\'s CSS import', () => {
