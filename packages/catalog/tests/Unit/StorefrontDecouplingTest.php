...
it('has no Markommerce\\Layout\\, Markommerce\\Frontend\\, or Markommerce\\ThemeBlank\\ imports in any file under packages/catalog/src', function (): void {
...
it('has no Markommerce\\Layout\\, Markommerce\\Frontend\\, Markommerce\\ThemeBlank\\, Marko\\Routing\\, or Marko\\View\\ imports in any file under packages/catalog/tests', function (): void {
...
it('drops marko/config from packages/catalog/composer.json require if and only if no file under packages/catalog/src imports the Marko\\Config\\ namespace', function (): void {
    $srcDir = dirname(__DIR__, 2) . '/src';
    $files = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($srcDir));
    ...
    if ($hasConfigImport) {
        expect($require)->toHaveKey('marko/config', 'marko/config is used in src/ but missing from composer.json require');
    } else {
        expect($require)->not->toHaveKey('marko/config', 'marko/config is not used in src/ so it must not appear in composer.json require');
    }
});
