...
it('skips its own file (basename MarketDecouplingTest.php) when walking the catalog source and test trees, mirroring ScopeDecouplingTest\'s self-exclusion at line 66', function (): void {
...
it('finds no occurrences of CategoryTreeMarketAssignment in any catalog source or test file (excluding self)', function (): void {
...
it('finds no occurrences of assignTreeToMarket, resolveTreeForMarket, or unassignMarket in any catalog source or test file (excluding self)', function (): void {
...
it('finds no occurrences of TreeHasMarketAssignmentsException in any catalog source or test file (excluding self)', function (): void {
...
it('finds no occurrences of the Markommerce\\CatalogMarketCategoryTrees namespace in any catalog source or test file (excluding self)', function (): void {
...
it('finds no occurrences of the Markommerce\\Market or Markommerce\\CatalogMarket namespaces in any catalog source or test file (excluding self)', function (): void {
    $violations = collectViolations(['Markommerce\\Market\\', 'Markommerce\\CatalogMarket\\']);

    expect($violations)->toBeEmpty(
        'Files containing Markommerce\\Market\\ or Markommerce\\CatalogMarket\\ namespaces: ' . implode(', ', $violations),
    );
});
