...
- [ ] `it returns null from load when the config_key row does not exist`
- [ ] `it returns a hydrated ConfigRow with decoded value and overrides from load when the row exists`
- [ ] `it returns an empty array from loadMany when no requested keys exist`
- [ ] `it returns a map keyed by config_key with one entry per existing row from loadMany`
- [ ] `it inserts a new row via compareAndSave when expectedVersion is 0 and no row exists`
- [ ] `it updates an existing row via compareAndSave when the stored version matches expectedVersion`
- [ ] `it bumps version by exactly 1 on every successful compareAndSave`
- [ ] `it returns false from compareAndSave when the stored version does not match expectedVersion`
- [ ] `it deletes the row when compareAndSave persists a row with null value and empty overrides`
- [ ] `it sets updated_at to NOW() on every successful compareAndSave`
- [ ] `it is safe under concurrent compareAndSave calls — only one of two simultaneous writers with the same expectedVersion succeeds`
- [ ] `it returns true as a no-op when compareAndSave persists an empty row against an absent key with expectedVersion 0`
- [ ] `it returns false from compareAndSave when expectedVersion is greater than 0 but the row no longer exists (deleted by another writer)`
- [ ] `it serializes a concurrent empty-mutation (delete) and non-empty mutation (override-add) to the same key without losing the non-empty mutation`
- [ ] `it serializes two simultaneous INSERTs to a brand-new key — exactly one succeeds, the other returns false`
