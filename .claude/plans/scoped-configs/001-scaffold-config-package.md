...
- [x] `it has a composer.json declaring markommerce/config with PHP 8.5 requirement`
- [x] `it declares marko-module type with the marko.module extra flag`
- [x] `it requires markommerce/scope as a self-version dependency`
- [x] `it autoloads the Markommerce\Config namespace from src/`
- [x] `it ships an empty module.php returning a valid bindings array`
- [x] `it is discoverable via composer dump-autoload from the workspace root`
- [x] `the root composer.json require block lists markommerce/config: self.version`
