...
- [ ] `it returns a string unchanged when target type is string`
- [ ] `it returns an int unchanged when target type is int`
- [ ] `it returns a float unchanged when target type is float`
- [ ] `it returns a bool unchanged when target type is bool`
- [ ] `it returns an array unchanged when target type is array`
- [ ] `it converts a backed-enum stored value to the enum instance via tryFrom`
- [ ] `it throws InvalidConfigValueException when the stored value is a string but target type is int`
- [ ] `it throws InvalidConfigValueException when a backed enum tryFrom returns null`
- [ ] `it throws InvalidConfigValueException carrying the key, raw value, and declared type in the message`
