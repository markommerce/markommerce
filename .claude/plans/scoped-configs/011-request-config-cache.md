...
- [ ] `it caches the resolved value on first call and returns the cached value on second call for the same config-key and scope projection`
- [ ] `it caches resolved null values and does not re-invoke the loader on subsequent reads of the same key`
- [ ] `it caches independently for the same config under different ScopeContext projections`
- [ ] `it projects the cache key over only the config's declared axes ignoring unrelated active axes`
- [ ] `it invalidates all cache entries for a given config key when invalidatePrefix is called`
- [ ] `it does not invalidate entries for unrelated config keys when invalidatePrefix is called`
- [ ] `it clears every entry when clear() is called`
- [ ] `CachingConfigResolver delegates to the wrapped ConfigResolver on cache miss and stores the result`
- [ ] `CachingConfigResolver returns the cached value without invoking the wrapped resolver on cache hit`
- [ ] `ConfigCacheResetMiddleware calls cache.clear() before forwarding to the next handler`
