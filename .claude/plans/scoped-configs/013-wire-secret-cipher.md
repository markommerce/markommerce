...
- [x] `it encrypts the value with SecretCipher when writing a #[Config(secret: true)] global`
- [x] `it encrypts the value with SecretCipher when writing a #[Config(secret: true)] override`
- [x] `it decrypts the stored ciphertext when resolving a #[Config(secret: true)] global`
- [x] `it decrypts the stored ciphertext when resolving a #[Config(secret: true)] override`
- [x] `it does not invoke the cipher when the property is not marked secret`
- [x] `it round-trips a secret value through write then read using SodiumSecretCipher with a real 32-byte key`
