...
- [ ] `it persists a new global value via setGlobal when the key has no existing row`
- [ ] `it replaces an existing global value via setGlobal preserving any per-scope overrides on the same key`
- [ ] `it removes the global value via unsetGlobal while keeping per-scope overrides intact`
- [ ] `it persists a new per-scope override via setOverride keyed by the signature string`
- [ ] `it replaces an existing per-scope override for the same signature`
- [ ] `it removes a specific per-scope override via unsetOverride leaving other overrides untouched`
- [ ] `it throws ConfigNotFoundException when writing to a key that no ConfigDefinition exists for`
- [ ] `it throws AxisNotDeclaredException when setOverride's signature uses an axis not declared on the property`
- [ ] `it retries up to 3 times on compareAndSave version conflict before throwing StaleConfigWriteException`
- [ ] `it bumps the row version after a successful write (visible via subsequent load)`
- [ ] `it returns successfully when unsetOverride is called for a signature that was never set (idempotent no-op via storage contract)`
- [ ] `it produces an identical row mutation when setGlobal is called with null as when unsetGlobal is called`
- [ ] `it produces an identical row mutation when setOverride is called with null as when unsetOverride is called`
- [ ] `it accepts SecretCipherInterface as a constructor dependency but does NOT invoke it for non-secret writes in this task's tests`
