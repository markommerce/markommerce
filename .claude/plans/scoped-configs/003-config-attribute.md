...
- [ ] `it constructs with a required key string`
- [ ] `it defaults the secret flag to false when omitted`
- [ ] `it accepts secret: true for properties needing encryption-at-rest`
- [ ] `it rejects an empty key string with a clear exception at construction time`
- [ ] `it is targetable to properties only (TARGET_PROPERTY)`
- [ ] `it is a readonly class so the metadata cannot mutate after construction`
