...
- [ ] `it generates a subclass extending the original config class`
- [ ] `it generates a property hook get clause for every #[Config] property`
- [ ] `it preserves the original property's declared type in the override`
- [ ] `it injects ConfigResolver as the only constructor dependency named __resolver`
- [ ] `it writes the source to a file path mirroring the original namespace under the target directory`
- [ ] `it creates intermediate directories when the target path nests`
- [ ] `it throws InvalidConfigClassException when a property is readonly`
- [ ] `it throws InvalidConfigClassException when a property has a union type`
- [ ] `it throws InvalidConfigClassException when the config class declares a constructor with required parameters`
- [ ] `it emits enum-typed property hooks using a leading-backslash FQN for the enum type`
- [ ] `it produces source that PHP can parse and require (require + class_exists assertion in a unique-class-per-test fixture)`
