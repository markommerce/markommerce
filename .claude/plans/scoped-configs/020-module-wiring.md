...
- [x] `it binds ConfigStorageInterface to InMemoryConfigStorage by default`
- [x] `it registers PreferenceRegistry as an instance in the container during boot`
- [x] `it discovers config classes by scanning module src directories for properties with #[Config]`
- [x] `it builds the ConfigRegistry at boot from the discovered class list`
- [x] `it registers the ProxyAutoloader at boot so generated proxies resolve`
- [x] `it does NOT instantiate SecretCipher at boot — only on first encrypt/decrypt invocation`
- [x] `it throws a loud setup exception at the FIRST secret read/write when MARKOMMERCE_CONFIG_SECRET_KEY is unset`
- [x] `it verifies the five #[Command]-annotated command classes exist under src/Command/ (Marko auto-discovers them)`
- [x] `it registers ConfigCacheResetMiddleware in globalMiddleware so RequestConfigCache is cleared per request`
- [x] `it binds SecretCipherInterface lazily via a closure that reads the 32-byte key from ConfigRepositoryInterface on first call`
- [x] `it regenerates a stale proxy at boot when markommerce.config.auto_regenerate is true and the config class source is newer than the proxy file`
- [x] `it does NOT regenerate any proxy at boot when markommerce.config.auto_regenerate is false`
- [x] `it generates a missing proxy at boot when markommerce.config.auto_regenerate is true and the proxy file is absent`
- [x] `it bubbles up InvalidConfigClassException from dev-mode regeneration so boot fails loudly`
- [x] `it binds ConfigResolver to the CachingConfigResolver decorator so all consumers get caching by default`
