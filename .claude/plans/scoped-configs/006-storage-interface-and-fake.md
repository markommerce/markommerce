...
- [ ] `it returns null from load when the key is not stored`
- [ ] `it returns the row from load after a compareAndSave with expectedVersion 0`
- [ ] `it bumps version by one on each successful compareAndSave`
- [ ] `it returns false from compareAndSave when expectedVersion does not match the stored version`
- [ ] `it leaves the stored row unchanged after a failed compareAndSave`
- [ ] `it returns multiple rows from loadMany in a single call`
- [ ] `it omits keys absent from storage in the loadMany result map`
- [ ] `it deletes the row when compareAndSave persists a row with null value and empty overrides`
- [ ] `it returns true from compareAndSave as a no-op when persisting an empty row against an absent key with expectedVersion 0`
