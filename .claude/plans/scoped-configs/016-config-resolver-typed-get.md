...
- [ ] `it returns an instance of the generated proxy class for a non-preferenced config class`
- [ ] `it returns an instance of the preferred proxy class when a Preference is registered via PreferenceRegistry`
- [ ] `it instantiates the proxy with the resolver as the __resolver dependency`
- [ ] `it throws ProxyNotGeneratedException with a config:generate suggestion when the proxy class is missing`
- [ ] `it throws InvalidConfigClassException when the preferred class is not a subclass of the requested config class`
- [ ] `it returns instances whose property hooks delegate through to ConfigResolver::resolved() for each field`
