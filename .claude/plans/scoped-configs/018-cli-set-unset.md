...
- [ ] `it sets a global value via config:set when no --scope flag is provided`
- [ ] `it sets a scoped override via config:set when --scope flag is provided`
- [ ] `it parses the shell <value> argument according to the property's declared type`
- [ ] `it rejects an unparseable value with a clear error citing the expected type`
- [ ] `it clears the global value via config:unset without --scope`
- [ ] `it clears a specific scoped override via config:unset with --scope`
- [ ] `it exits non-zero on StaleConfigWriteException without retrying`
- [ ] `it exits non-zero on AxisNotDeclaredException citing declared axes`
