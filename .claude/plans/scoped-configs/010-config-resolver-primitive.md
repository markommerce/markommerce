...
- [ ] `it returns the property default when no row exists for the config key`
- [ ] `it returns the global value cast to the declared type when the row has a global but no matching overrides`
- [ ] `it returns the override value when a matching signature exists in the row for the current ScopeContext`
- [ ] `it falls back from override to global when no override matches the current ScopeContext`
- [ ] `it falls back from global to default when the global is null and no override matches`
- [ ] `it preserves the most-specific override priority via OverrideMatcher when both single-axis and composite overrides are present`
- [ ] `it casts stored ints to int and stored strings to string`
- [ ] `it propagates InvalidConfigValueException unchanged from ValueCaster`
- [ ] `it throws ConfigNotFoundException when the configClass + field pair is not in the ConfigRegistry`
- [ ] `it resolves under a synthetic ScopeContext passed to resolvedAt without reading from the injected live context`
