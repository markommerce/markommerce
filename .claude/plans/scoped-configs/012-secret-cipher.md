...
- [x] `it round-trips a plaintext string through encrypt and decrypt (SodiumSecretCipher)`
- [x] `it produces a different ciphertext for the same plaintext on each call (nonce randomization)`
- [x] `it throws a setup exception when constructed if the sodium extension is unavailable`
- [x] `it throws an exception when constructed with a key of the wrong length`
- [x] `it throws an exception on decrypt when the ciphertext is malformed or tampered`
- [x] `it returns base64 output from encrypt so the ciphertext is safe to embed in JSON`
- [x] `NullSecretCipher throws SecretCipherException::notConfigured from encrypt and decrypt`
- [x] `NullSecretCipher can be constructed without arguments so it can serve as a default container binding`
