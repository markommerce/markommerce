...
- [x] `it lists every registered config key with its source class and axes in human-readable output`
- [x] `it lists every registered config key as JSON when --format=json is given`
- [x] `it returns the resolved global value for a key under an empty scope`
- [x] `it returns the resolved scoped value for a key when --scope is given`
- [x] `it exits non-zero with a did-you-mean hint when the key is unknown`
- [x] `it does not print decrypted plaintext when the config is #[Config(secret: true)] — instead shows a redacted marker like ***`
