...
- [x] `it builds a registry from a single config class with one #[Config] property`
- [x] `it builds a registry from multiple config classes`
- [x] `it captures the property's declared PHP type as a normalized string in ConfigDefinition`
- [x] `it captures the property's default value in ConfigDefinition`
- [x] `it captures axes from #[Scoped] when present and uses an empty axes list when absent`
- [x] `it throws ConfigKeyConflictException when two properties declare the same #[Config(key)]`
- [x] `it throws AxisNotDeclaredException when a property's #[Scoped] axis is not in the ScopeRegistry`
- [x] `it throws InvalidConfigClassException when a #[Config] property has a union or intersection type`
- [x] `it throws InvalidConfigClassException when a #[Config] property is non-nullable and has no default value`
- [x] `it throws InvalidConfigClassException when the config class declares a constructor with at least one required parameter`
- [x] `it accepts a config class whose constructor has only optional/defaulted parameters`
- [x] `it throws ConfigNotFoundException when registry.definition(class, field) is called for unknown fields`
- [x] `it returns a definition by string key via registry.byKey(key)`
