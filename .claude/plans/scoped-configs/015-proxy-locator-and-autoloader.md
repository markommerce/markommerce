...
- [ ] `it maps an original class FQN to a deterministic generated proxy FQN`
- [ ] `it locates the same proxy FQN regardless of leading backslash variations in the original FQN`
- [ ] `it autoloads a generated proxy class from a file under the target directory`
- [ ] `it returns false from the autoloader when the requested class is not under the Generated namespace`
- [ ] `it expands a list of registered classes by adding preference subclasses returned by PreferenceRegistry::getPreference`
- [ ] `it follows nested preferences via PreferenceRegistry::getPreference (which already chains)`
- [ ] `it throws InvalidConfigClassException when a preferred class is not a subclass of the original config class`
- [ ] `it propagates PreferenceConflictException::circularPreference unchanged when PreferenceRegistry detects a cycle (no duplicate cycle detection in the scanner)`
