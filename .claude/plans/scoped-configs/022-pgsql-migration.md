...
- [ ] `it creates the config_values table with config_key as primary key`
- [ ] `it creates the value column as JSONB and nullable`
- [ ] `it creates the overrides column as JSONB defaulting to empty object`
- [ ] `it creates the version column as integer defaulting to 0`
- [ ] `it creates the updated_at column as timestamptz`
- [ ] `it creates a GIN index on the overrides column`
- [ ] `it is idempotent across multiple runs (no duplicate index / table errors)`
