...
- [ ] `it generates a proxy file for every registered config class`
- [ ] `it expands the class list via PreferenceAwareScanner so preferenced subclasses also get proxies`
- [ ] `it clears the target directory before writing new proxies`
- [ ] `it writes proxy files at FQN-mirroring paths under the target directory`
- [ ] `it prints a summary line counting generated proxies`
- [ ] `it exits non-zero when ProxyGenerator throws InvalidConfigClassException`
- [ ] `it warns and exits zero when the registry is empty`
