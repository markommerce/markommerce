...
|------|-------------|------------|--------|
| 001 | Scaffold `markommerce/config` package | - | completed |
| 002 | Exception classes | 001 | completed |
| 003 | `#[Config]` attribute | 001 | completed |
| 004 | `ConfigDefinition` + `ConfigRow` value objects | 001 | completed |
| 005 | `ConfigRegistry` + `ConfigRegistryBuilder` (reflection scan) | 002, 003, 004 | completed |
| 006 | `ConfigStorageInterface` + `InMemoryConfigStorage` | 002, 004 | completed |
| 007 | `ValueCaster` (strict cast / throw on mismatch) | 002, 004 | completed |
| 008 | `OverrideMatcher` (adapt scope walker to single-row JSON-map) | 002, 004 | completed |
| 009 | `ConfigWriter` (CRUD + optimistic lock + write-time axis validation; takes `SecretCipherInterface` as ctor dep with `NullSecretCipher` default) | 005, 006, 012 | completed |
| 010 | `ConfigResolver::resolved()` primitive (full ctor: registry, storage, matcher, caster, context, cipher, locator, prefRegistry) | 005, 006, 007, 008, 012 | completed |
| 011 | `ConfigCacheInterface` + `RequestConfigCache` + `ConfigCacheResetMiddleware` | 010 | completed |
| 012 | `SecretCipherInterface` + `SodiumSecretCipher` + `NullSecretCipher` stub | 002 | completed |
| 013 | Wire secret-branch behavior into `ConfigWriter` + `ConfigResolver` (no ctor changes — already wired in tasks 009/010) | 009, 010, 012 | completed |
| 014 | `ProxyGenerator` + `ProxyWriter` (codegen of `*_Resolved` subclasses) | 005 | completed |
| 015 | `ProxyLocator` + `ProxyAutoloader` + `PreferenceAwareScanner` (uses Marko's `PreferenceRegistry`) | 014 | completed |
| 016 | `ConfigResolver::get()` typed entry point (returns proxy instance) — ctor unchanged from task 010 | 010, 015 | completed |
| 017 | CLI: `config:list` + `config:get` (Marko `#[Command]` auto-discovery) | 010 | completed |
| 018 | CLI: `config:set` + `config:unset` (Marko `#[Command]` auto-discovery) | 009 | completed |
| 019 | CLI: `config:generate` (Marko `#[Command]` auto-discovery) | 014, 015 | completed |
| 020 | `module.php` final wiring (bindings, singletons, `PreferenceRegistry` instance, `ConfigClassDiscovery`, lazy `SecretCipher`) | 011, 013, 016, 017, 018, 019 | completed |
| 021 | Scaffold `markommerce/config-pgsql` package | 006 | completed |
| 022 | `config_values` table migration + schema emitter | 021 | completed |
| 023 | `PgsqlConfigStorage` implementation + integration tests | 021, 022 | completed |
| 024 | README for `markommerce/config` | 002–020 | completed |
| 025 | README for `markommerce/config-pgsql` | 021, 022, 023 | completed |
