...
Pricing pipeline for Markommerce. `markommerce/pricing` resolves a product's effective price as a `Money` value object. The base implementation reads `Product.priceAmount` via `ScopeResolver` (honoring the market scope through the `ProductScopedOverrides` companion) and pairs the amount with the active base currency from `CurrencyResolver`. `PriceResolverInterface` is the primary Marko `#[Plugin]` seam for layering additional pricing concerns --- sale prices, tier prices, customer-group discounts, and promotional overrides --- without modifying the core implementation.

Tax mode is resolved independently via `markommerce/tax`; `pricing` has no dependency on `tax`.
...
The package binds `PriceResolverInterface` to `PriceResolver` automatically via `module.php`.
...
- [markommerce/catalog](/docs/packages/catalog/) --- `Product` entity and `priceAmount` column
- [markommerce/catalog-market](/docs/packages/catalog-market/) --- Registers `Product.priceAmount` on the `market` axis, enabling per-market price overrides
- [markommerce/tax](/docs/packages/tax/) --- Independent tax mode resolution; not a dependency of `pricing`
